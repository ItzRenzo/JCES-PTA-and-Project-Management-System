...
<head>
    ...
    <title>Teacher Dashboard - {{ config('app.name', 'JCES-PTA') }}</title>
        ...
        <div id="sidebar" class="sidebar bg-gradient-to-b from-green-500 to-green-600 flex flex-col overflow-hidden">
            ...
            <nav class="flex-1 px-3 py-6 space-y-2">
                <a href="{{ route('teacher.dashboard') }}"
                   class="sidebar-link flex items-center gap-3 px-4 py-3 text-white {{ request()->routeIs('teacher.dashboard') ? 'bg-green-700' : 'hover:bg-green-700' }} rounded-lg font-medium transition-colors">
                ...
                <a href="{{ route('teacher.announcements') }}"
                   class="sidebar-link flex items-center gap-3 px-4 py-3 text-white {{ request()->routeIs('teacher.announcements') ? 'bg-green-700' : 'hover:bg-green-700' }} rounded-lg font-medium transition-colors">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                ...
                <a href="{{ route('teacher.payments.index') }}"
                   class="sidebar-link flex items-center gap-3 px-4 py-3 text-white {{ request()->routeIs('teacher.payments.*') ? 'bg-green-700' : 'hover:bg-green-700' }} rounded-lg font-medium transition-colors">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                ...
                <a href="{{ route('teacher.users') }}"
                   class="sidebar-link flex items-center gap-3 px-4 py-3 text-white {{ request()->routeIs('teacher.users') ? 'bg-green-700' : 'hover:bg-green-700' }} rounded-lg font-medium transition-colors">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                ...
                <a href="{{ route('profile.edit') }}" class="sidebar-profile flex items-center gap-3 px-4 py-3 text-white hover:bg-green-700 rounded-lg font-medium transition-colors w-full">
                    <div class="w-10 h-10 bg-white rounded-full flex items-center justify-center">
                        <span class="text-green-600 font-semibold text-sm">
                            {{ strtoupper(substr(Auth::user()->first_name ?? Auth::user()->username ?? 'T', 0, 1) . substr(Auth::user()->last_name ?? '', 0, 1)) }}
                        </span>
                    </div>
                    <div class="text-white sidebar-label" x-cloak>
                        <div class="text-sm font-semibold">{{ Auth::user()->name ?? Auth::user()->username ?? 'Teacher' }}</div>
